...
def setup_commands(bot):
    ...
    async def server_autocomplete(
        ...
        return [
            app_commands.Choice(name=f"{sid} - {bot.mc.get_server_config(sid)['name']}", value=sid)
            for sid in servers
            if current.lower() in sid.lower() or current.lower() in bot.mc.get_server_config(sid)['name'].lower()
        ][:25]
