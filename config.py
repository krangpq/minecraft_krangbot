...
if __name__ == '__main__':
    ...
    "creative": {
        "name": "크리에이티브 서버",
        "description": "건축 서버",
        "path": r"C:\minecraft\creative" if OS_TYPE == "Windows" else "/home/minecraft/creative",
        "start_command": "java -Xmx2G -Xms1G -jar server.jar nogui",
        "stop_command": "stop",
        "port": 25566,
        "rcon": {
            "enabled": False,
...
# 알림을 받을 디스코드 채널 ID
# 여러 서버/채널에 알림을 보낼 수 있음
NOTIFICATION_CHANNELS = {
    "main": 0,      # 메인 서버 알림 채널 ID
    "creative": 0,  # 크리에이티브 서버 알림 채널 ID
}

# 알림 종류별 활성화 여부
NOTIFICATIONS = {
    "server_start": True,      # 서버 시작
    "server_stop": True,       # 서버 중지
    "server_crash": True,      # 서버 크래시
    "player_join": True,       # 플레이어 접속
    "player_leave": True,      # 플레이어 퇴장
    "backup_complete": True,   # 백업 완료
    "performance_warning": True  # 성능 경고
}
...
# 서버 제어 권한 (Discord 역할 또는 권한)
# "manage_guild": 서버 관리 권한 필요
# "administrator": 관리자 권한 필요
REQUIRED_PERMISSION = "manage_guild"
...
if __name__ == '__main__':
    ...
    # 서버 설정 확인
    print(f"\n🎮 마인크래프트 서버: {len(MINECRAFT_SERVERS)}개")
    for server_id, server_config in MINECRAFT_SERVERS.items():
        print(f"\n[{server_id}] {server_config['name']}")
        server_path = Path(server_config['path'])
        if server_path.exists():
            print(f"  ✅ 경로 존재: {server_path}")
        else:
            print(f"  ❌ 경로 없음: {server_path}")
        print(f"  포트: {server_config['port']}")
        print(f"  RCON: {'활성화' if server_config['rcon']['enabled'] else '비활성화'}")
    
    print("\n" + "=" * 60)
    print("\n💡 설정이 완료되지 않았다면:")
    print("1. TOKEN과 BOT_OWNER_ID 입력")
    print("2. 마인크래프트 서버 경로 확인")
    print("3. 서버 설정 (포트, RCON 등) 확인")
    print("=" * 60)