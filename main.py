...
if __name__ == '__main__':
    ...
    async def on_ready(self):
        ...
        try:
            ...
            print(f"⚠️ 경고: BOT_OWNER_ID({BOT_OWNER_ID})가 올바르지 않습니다!")
            print("   config.py에서 본인의 디스코드 ID로 변경해주세요.")
        ...
        # 서버 목록 출력
        print(f"\n📋 관리 중인 서버: {len(MINECRAFT_SERVERS)}개")
        for server_id, config in MINECRAFT_SERVERS.items():
            status = "🟢 실행 중" if self.mc.is_server_running(server_id) else "🔴 중지됨"
            print(f"   [{server_id}] {config['name']} - {status}")
